...
export const convertToFormat = (
  ...
  return new Promise((resolve, reject) => {
    ...
    const expectedOutputFileName = inputFileName.replace(/\.[^/.]+$/, `.${outputFormat}`);
      ...
      (error, _stdout, stderr) => {
        if (error) {
          ...
          setTimeout(() => {
            ...
            const convertedFile = files.find(file => file.endsWith(`.${outputFormat}`));
            ...
            if (convertedFile) {
              ...
              reject(new Error(`Aucun fichier ${outputFormat} n'a été trouvé dans le répertoire de sortie`));
            }
          }, 1000);
        }
      }
